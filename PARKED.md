...
| 2026-08-14 | **MBTI's middle two letters are decided by sun sign alone.** With the seed now passed, all 16 types are reachable across a population — but per sun sign only 4 to 8. Air always lands NF, fire always NT, and the three questions only move E/I and J/P. So on those two axes MBTI is not an independent lens, it is astrology in different letters — two of the six frameworks partly collapsed into one. Fixing it means deciding what each element says about openness and agreeableness, which is a content decision, not a silent edit. |
