...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🔄 Identity Flexibility Analyst: Processing analysis...');
...
`;
    ...
    const openAIResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert identity adaptability analyst specializing in narrative flexibility and reinvention patterns.' },
    ...
    console.log('✅ Identity Flexibility Analysis completed, length:', content.length);

    return new Response(JSON.stringify({ content }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('❌ Identity Flexibility Analyst error:', error);
