  ...
  try {
    ...
    try {
      ...
      if (searchError.message.includes("rate limit")) {
        ...
        // Fallback to regular GPT-4o without search
        const { blueprint, rawResponse } = await generateBlueprintWithAI(userMeta);
        ...
        return new Response(
          JSON.stringify({ 
            ...
            blueprint,
            rawResponse,
            notice: "Generated with GPT-4o (fallback from search preview due to rate limits)"
          }),
          { 
            headers: { 
              ...corsHeaders, 
        ...
        // If it's not a rate limit error, rethrow it for general error handling
        throw searchError;
      }
    }
...
/**
 * Generate a blueprint using regular GPT-4o (without search)
...
6. Bashar spiritual principles`;
  ...
  console.log("Calling OpenAI with GPT-4o (no search)");
  ...
  console.log("Received blueprint from OpenAI (regular GPT-4o)");
