...
export async function calculatePlanetaryPositions(
  ...
  try {
    ...
    // Parse the birth date and time using native Date and date-fns-tz
    let utcDateTime: Date;
    
    if (birthTime) {
      // If birth time is provided, use the full datetime
      const localDateTimeString = `${birthDate}T${birthTime}`;
      const localDateTime = new Date(localDateTimeString);
      
      // Convert from local timezone to UTC using date-fns-tz
      try {
        utcDateTime = fromZonedTime(localDateTime, tzId);
      } catch (e) {
        console.error(`Error converting timezone, using fallback: ${e}`);
        // Fallback to assuming the time is already in UTC
        utcDateTime = new Date(`${birthDate}T${birthTime}:00.000Z`);
      }
    } else {
      // If no birth time, default to noon UTC
      utcDateTime = new Date(`${birthDate}T12:00:00.000Z`);
    }
    
    console.log(`Converted local time to UTC: ${utcDateTime.toISOString()}`);
...
function calculateJulianDayFromDateTime(dt: Date): number {
  ...
  const m = dt.getUTCMonth() + 1; // getUTCMonth() returns 0-11, we need 1-12
