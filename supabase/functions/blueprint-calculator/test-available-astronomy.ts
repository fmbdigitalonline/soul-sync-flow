...
async function testAvailableAstronomy(req) {
  ...
  try {
    ...
    try {
      const luxon = await import('https://esm.sh/luxon@3.4.4');
    ...
    for (const libName of importMapLibs) {
      try {
        ...
        if (libName === 'astronomia') {
          lib = await import('https://esm.sh/astronomia@4.1.1');
        } else if (libName === 'astronomy-engine') {
          lib = await import('https://esm.sh/astronomy-engine@2');
        } else if (libName === 'luxon') {
          lib = await import('https://esm.sh/luxon@3.4.4');
        }
