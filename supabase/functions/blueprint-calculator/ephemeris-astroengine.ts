...
try {
  ...
  throw new Error(`Failed to import astronomy-engine: ${errorMessage}`);
}
...
export async function calculatePlanetaryPositionsWithAstro(
  ...
  try {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown self-test error';
      throw new Error(`Astronomy engine self-test failed: ${errorMessage}`);
    }
    ...
    try {
      ...
      throw new Error(`Failed to geocode location: ${location}. Error: ${errorMessage}`);
    }
    ...
    for (const body of bodies) {
      try {
        ...
        throw new Error(`Planetary calculation failed for ${body.id}: ${errorMessage}`);
      }
    }
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown lunar nodes error';
      throw new Error(`Lunar nodes calculation failed: ${errorMessage}`);
    }
    ...
    try {
      ...
      try {
        ...
        throw new Error(`House calculations completely failed: ${fallbackErrorMessage}`);
      }
    }
