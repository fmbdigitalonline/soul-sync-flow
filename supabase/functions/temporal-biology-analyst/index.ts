...
serve(async (req) => {
  ...
  try {
    ...
    console.log('⏰ Temporal Biology Analyst: Processing analysis...');
...
`;
    ...
    const openAIResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert chronobiology analyst specializing in temporal patterns and biological rhythm analysis.' },
    ...
    console.log('✅ Temporal Biology Analysis completed, length:', content.length);

    return new Response(JSON.stringify({ content }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('❌ Temporal Biology Analyst error:', error);
