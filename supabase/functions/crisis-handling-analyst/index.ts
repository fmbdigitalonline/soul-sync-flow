...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🆘 Crisis Handling Analyst: Processing analysis...');
...
`;
    ...
    const openAIResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert crisis analyst specializing in stress response patterns and resilience mechanisms.' },
    ...
    console.log('✅ Crisis Handling Analysis completed, length:', content.length);

    return new Response(JSON.stringify({ content }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('❌ Crisis Handling Analyst error:', error);
