...
const SimplifiedBlueprintViewer: React.FC<SimplifiedBlueprintViewerProps> = ({ blueprint }) => {
  ...
  const SectionHeader = ({ title, isExpanded, onClick }: { title: string; isExpanded: boolean; onClick: () => void }) => (
    <button
      onClick={onClick}
      className={`w-full flex items-center justify-between ${spacing.button} ${getTextSize('text-base')} font-semibold text-left hover:bg-muted/50 ss-rs transition-colors`}
    >
      <span>{title}</span>
          ...
          <CardContent className={`${spacing.card} pt-0`}>
            <div className="grid grid-cols-1 gap-4 w-full max-w-full">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div 
                  className="text-center p-3 bg-[var(--ss-accent-wash)] ss-r cursor-pointer hover:bg-[var(--ss-accent-wash-2)] transition-colors active:scale-[0.98]"
                  onClick={() => {
          ...
          <CardContent className={`${spacing.card} pt-0`}>
            <div className="space-y-4 w-full max-w-full">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div 
                  className="p-3 bg-[var(--ss-accent-wash)] ss-r text-center cursor-pointer hover:bg-[var(--ss-accent-wash-2)] transition-colors active:scale-[0.98]"
                  onClick={() => {
                    const desc = getDescription('mbtiDescriptions', mbtiType);
                    openDetailModal({
                      title: desc.fullTitle,
        ...
        {expandedSections.astrology && (
          <CardContent className={`${spacing.card} pt-0`}>
            <div className="space-y-4 w-full max-w-full">
              ...
              <div 
                className="mt-4 p-3 bg-[var(--ss-accent-wash)] ss-r text-center cursor-pointer hover:bg-[var(--ss-accent-wash-2)] transition-colors active:scale-[0.98]"
                onClick={() => {
                  const desc = getDescription('chineseZodiacDescriptions', chineseZodiac);
                  openDetailModal({
                    title: desc.fullTitle,
