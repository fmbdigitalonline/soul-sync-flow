...
const MainLayout: React.FC<MainLayoutProps> = ({
  ...
  return <div className="min-h-screen bg-background w-full">
      {isMobile ? <>
          {/* Mobile Header */}
          <div className="bg-white/80 backdrop-blur-lg sticky top-0 z-40 w-full shadow-sm">
            ...
            {isMenuOpen && user && <div className="bg-card/95 backdrop-blur-lg p-4 space-y-2 w-full">
          ...
          <div className="flex flex-1 min-h-0">
            {/* Desktop Sidebar */}
            {shouldShowDesktopNav && <aside className="w-64 min-h-full bg-card/80 backdrop-blur-lg flex-col flex">
                {/* Logo Section */}
              ...
              return <Link key={item.to} to={item.to} className={cn("flex items-center space-x-3 p-3 rounded-2xl transition-all duration-200 transition-shadow hover:shadow-sm font-cormorant font-medium", isActive(item.to) ? "bg-gradient-to-r from-primary/10 to-accent/10 text-primary font-semibold" : "text-muted-foreground hover:bg-accent/50 hover:text-primary")}>
            ...
            <aside className="hidden lg:block w-72 min-h-full p-4"></aside>
          </div>
        </>}
