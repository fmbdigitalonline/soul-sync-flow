  ...
  return (
    <div className={`transition-all duration-500 w-full max-w-full overflow-hidden ${
      ...
      <div className="w-full max-w-full overflow-hidden">
        <div className={`flex items-start gap-3 w-full max-w-full ${isFoldDevice ? 'flex-col' : ''}`}>
          ...
          <div className="flex-1 min-w-0 w-full max-w-full overflow-hidden">
            <div className={`flex items-center gap-2 mb-2 flex-wrap w-full max-w-full ${isFoldDevice ? 'justify-center' : ''}`}>
              <h3 className={`font-semibold text-gray-800 flex-shrink-0 ${getTextSize('text-sm')}`}>Soul Coach Guidance</h3>
              <Badge variant="outline" className={`flex-shrink-0 ${getTextSize('text-xs')}`}>
                Step {tourStep + 1} of {totalSteps}
              </Badge>
            </div>
            <p className={`text-gray-700 leading-relaxed mb-3 w-full max-w-full break-words ${getTextSize('text-xs')} ${isFoldDevice ? 'text-center' : ''}`}>
              Getting you oriented with your personalized journey...
            </p>
            <div className={`flex gap-2 w-full max-w-full ${isFoldDevice ? 'flex-col' : 'justify-end'}`}>
              <Button 
                variant="ghost"
                onClick={onSkipTour}
                size="sm"
                className={`text-gray-500 hover:text-gray-700 rounded-xl flex-shrink-0 ${getTextSize('text-xs')} ${touchTargetSize} ${isFoldDevice ? 'w-full' : ''}`}
              >
                Skip Tour
              </Button>
              <Button 
                onClick={onNextStep}
                size="sm"
                className={`bg-soul-purple hover:bg-soul-purple/90 text-white rounded-xl flex-shrink-0 ${getTextSize('text-xs')} ${touchTargetSize} ${isFoldDevice ? 'w-full' : ''}`}
              >
