...
export class ConversationPhaseTracker {
  ...
  private static getAllowedNextClusters(cluster: ConversationCluster): ConversationCluster[] {
    const route = CONVERSATION_STATE_SCHEMA.routing_graph.find(r => r.from === cluster);
    return (route?.to || []) as ConversationCluster[];
  }
  
  // Backward-compatible method for existing code
  static detectPhase(conversationHistory: any[]): ConversationCluster {
    const lastUserMessage = conversationHistory[conversationHistory.length - 1]?.content || '';
    const result = this.detectState(lastUserMessage, conversationHistory);
    return result.cluster;
  }
  
  // Phase guidance for LLM system prompt
  static getPhaseGuidance(cluster: ConversationCluster): string {
    const clusterDef = CONVERSATION_STATE_SCHEMA.clusters.find(c => c.id === cluster);
    
    return `
DETECTED STATE: ${cluster.toUpperCase()}
DESCRIPTION: ${clusterDef?.description || 'Unknown state'}
OPENING RULE: ${clusterDef?.opening_rule || 'Respond naturally'}
ALLOWED NEXT STATES: ${clusterDef?.allowed_next_clusters?.join(', ') || 'any'}
`;
  }
  
  // Get cluster-specific repetitive patterns for anti-repetition sentinel
  static getRepetitivePatterns(cluster: ConversationCluster): string[] {
    const patterns: Record<ConversationCluster, string[]> = {
      validation: ['that feeling', 'i hear that', 'it sounds like', 'the weight of', 'navigating', 'the tension between'],
      clarification: ['to understand', 'let me break', 'essentially', 'in other words', 'what this means'],
      decision: ['so the path', 'here\'s what', 'the next step', 'what you can do'],
      reflection: ['what you\'ve discovered', 'the key insight', 'looking back', 'what we\'ve explored'],
      exploration: ['what if', 'consider', 'another possibility', 'think about'],
      engagement: ['i sense', 'it seems', 'sounds like'],
      constraint: ['given the', 'within these limits', 'considering'],
      frustration: ['i understand that', 'that must be', 'it\'s frustrating'],
      meta_dialogue: ['let me adjust', 'i\'ll shift', 'adapting'],
      closure: ['glad to', 'happy to', 'pleased']
    };
