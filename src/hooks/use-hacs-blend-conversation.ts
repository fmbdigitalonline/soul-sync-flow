...
export const useHACSBlendConversation = () => {
  ...
  const sendMessage = async (content: string): Promise<void> => {
    ...
    try {
      ...
      // Route through Unified Brain Service (11 Hermetic Components) then to blend edge function
  ...
  const generateBlendQuestion = (intelligenceLevel: number): string => {
    const questions = [
      "How do you balance your practical goals with your spiritual growth?",
      "What integrated approach helps you stay both productive and aligned?",
      "How do you merge your inner wisdom with outer achievement?",
      "What does holistic success look like in your life?",
      "How do you maintain flow between work and soul fulfillment?",
      "What practices help you integrate all aspects of your being?"
    ];
  ...
  const determineResponseQuality = (hacsResponse: string, userMessage: string): 'excellent' | 'good' | 'average' | 'poor' => {
    ...
    if (hacsResponse.length > 100 && (hacsResponse.includes('balance') || hacsResponse.includes('integrate'))) {
      return 'excellent';
