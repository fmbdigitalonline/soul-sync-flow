  ...
  const continueCards = [
    {
      ...
      status: continueItem?.type === 'task' ? 'In Progress' : 'Set up next',
      description: continueItem?.title || 'Reconnect your routines and priorities.',
      actionPath: continueItem?.actionPath || '/tasks',
      icon: <Layers className="h-4 w-4" />,
      badge: continueItem?.lastActivity ? `Updated ${formatRelativeTime(continueItem.lastActivity)}` : 'Start'
    },
    {
      title: "Guided Discovery",
      status: guidanceInsight ? 'Ongoing' : 'Ready',
              ...
              <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                ...
                <div className="w-full lg:w-auto flex flex-col items-start gap-3">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <Button size="lg" className="h-touch px-6" onClick={() => navigate(continueItem?.actionPath || '/companion')}>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {continueCards.map(card => <div key={card.title} className="rounded-2xl border border-border/70 bg-muted/20 p-4 hover:bg-primary/5 transition-colors">
                    ...
                    <div className="flex items-center justify-between">
                      <Button variant="ghost" size="sm" className="px-0 text-primary" onClick={() => navigate(card.actionPath)}>
                        Resume
                        <ChevronRight className="h-4 w-4 ml-1" />
                      </Button>
                      <Button variant="outline" size="sm" className="h-9" onClick={() => navigate(card.actionPath)}>
                        Open track
                      </Button>
                    </div>
                  </div>)}
